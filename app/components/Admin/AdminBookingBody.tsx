...
import { get_all_sessions } from "@/services/consultant";
...
const AdminBookingBody = ({userType}: StudentBookingBodyProps) => {
    const {
        approveReschedule, 
        buttonLoader,
        makePayment,
    ...
    const viewBookingTrigger = (item: Booking): void => {
        genralStore.getState().setBooking(item);
        openModal("booking", "details-admin")
    }
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            try {
                const response = await get_all_sessions();
    ...
    return (
        <div className="container-3">
            <div>
                ...
                <p className="color-grey-text text-[.8rem]">The session timings are following the client's local timezone </p>
            </div>
                        ...
                        return (
                            <div className="booking-cont" key={index}>
                                <div className="flex items-start justify-between">
                                    <p className="w-[70%]">Mentorship session between <span className="color-darker font-bold">{`${item.consultant?.instructor?.user?.first_name} ${item.consultant?.instructor?.user?.last_name}`}</span> and <span className="color-darker font-bold">{item.user?.first_name} {item.user?.last_name}</span> </p>
...

                                {
                                    isAdminCancelled &&
                                    <div className="alert no notification error text-[.9rem]">
                                        <p className="color-error text-[.9rem] font-semibold">This session has been cancelled by the Admin</p>
                                    </div>
                                }
